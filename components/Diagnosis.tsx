...
const Diagnosis: React.FC<Props> = ({ user, setUser }) => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('km_diag_history');
    if (saved) setHistory(JSON.parse(saved));
  }, []);

  useEffect(() => {
    localStorage.setItem('km_diag_history', JSON.stringify(history));
  }, [history]);
  ...
  const handleDiagnose = async () => {
    ...
    try {
      ...
      let msg = error?.message || "Failed to diagnose plant health.";
      if (msg.includes("API key not valid") || msg.includes("API_KEY_INVALID") || msg.includes("PLACEHOLDER_API_KEY")) {
        msg = "Invalid or missing Gemini API Key. Please set a valid GEMINI_API_KEY in your .env.local file.";
      }
      setErrorMessage(msg);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-12 page-transition pb-24 relative px-4">
      ...
      {errorMessage && (
        <div className="bg-amber-50 border-2 border-amber-200 text-amber-900 p-6 rounded-3xl flex items-center justify-between gap-4 print:hidden shadow-sm">
          <div className="flex items-center gap-4">
            <AlertCircle className="text-amber-600 shrink-0" size={28} />
            <div>
              <p className="font-bold text-lg">AI Service Configuration Required</p>
              <p className="text-amber-700 text-sm font-medium">{errorMessage}</p>
            </div>
          </div>
          <button onClick={() => setErrorMessage(null)} className="p-2 text-amber-600 hover:bg-amber-100 rounded-full transition-all">
            <X size={20} />
          </button>
        </div>
      )}
