...
const VirtualFarm: React.FC = () => {
  ...
  const [irrigation, setIrrigation] = useState(() => localStorage.getItem('km_farm_irrigation') || 'Drip Irrigation');
  const [enrichment, setEnrichment] = useState(() => localStorage.getItem('km_farm_enrichment') || 'Organic Compost');
  const [pestPolicy, setPestPolicy] = useState(() => localStorage.getItem('km_farm_pest') || 'Biological Control');
  ...
  useEffect(() => {
    localStorage.setItem('km_farm_irrigation', irrigation);
    localStorage.setItem('km_farm_enrichment', enrichment);
    localStorage.setItem('km_farm_pest', pestPolicy);
  }, [irrigation, enrichment, pestPolicy]);

  useEffect(() => {
    const savedPlots = localStorage.getItem('kisaanmitra_farm_plots');
  ...
  useEffect(() => {
    if (plots.length > 0) localStorage.setItem('kisaanmitra_farm_plots', JSON.stringify(plots));
  }, [plots]);
