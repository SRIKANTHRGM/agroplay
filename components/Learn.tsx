                ...
                {cropData?.workflow?.map((step, i) => {
                  ...
                  return (
                    <button 
                      ...
                      className={`w-full text-left p-6 rounded-[2.5rem] transition-all flex items-center gap-6 group relative overflow-hidden ${
                        selectedStepIndex === i 
                    ...
                    >
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-lg flex-shrink-0 transition-all shadow-inner ${
                        selectedStepIndex === i ? 'bg-white/10 text-white' : verified ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-400'
                      }`}>
                ...
                <div className="aspect-video bg-slate-900 relative group overflow-hidden">
                   ...
                   <div className="absolute top-10 left-10 flex items-center gap-4">
                      <div className="px-6 py-2.5 bg-black/60 backdrop-blur-xl rounded-2xl text-white font-black text-[10px] uppercase tracking-widest border border-white/20">
                        {isVerified ? 'VERIFICATION RECORD' : 'SIMULATION MODE'}
                      </div>
          ...
          <div className="bg-white w-full max-w-4xl rounded-[4.5rem] shadow-[0_0_150px_rgba(34,197,94,0.4)] overflow-hidden flex flex-col md:flex-row max-h-[90vh] animate-in zoom-in-95 duration-500 relative border-8 border-white">
             ...
             <div className="w-full md:w-1/2 relative bg-slate-900 overflow-hidden group">
                ...
                {proofImage ? (
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center p-16 text-center space-y-10">
                     <div className="relative">
                        <div className="absolute inset-0 bg-green-500/20 rounded-full blur-3xl animate-pulse" />
                        <div className="w-40 h-40 bg-white/5 rounded-[3rem] border border-white/10 flex items-center justify-center text-white relative z-10 shadow-2xl">
                           <Camera size={80} strokeWidth={1} />
                        </div>
                     </div>
                     <div className="space-y-3">
                        <h4 className="text-3xl font-black text-white outfit uppercase tracking-tighter">Capture Bio-Proof</h4>
                        <p className="text-slate-500 font-bold text-sm uppercase tracking-widest leading-relaxed px-10">Document your work for the Gemini Neural Vision scanner.</p>
                     </div>
                     <button onClick={() => fileInputRef.current?.click()} className="px-12 py-6 bg-white text-slate-900 rounded-3xl font-black text-xs uppercase tracking-[0.3em] hover:bg-green-400 hover:text-white transition-all shadow-2xl active:scale-95">
                       Open Visual Uplink
                     </button>
                  </div>
                )}
             ...
             <div className="flex-1 p-12 md:p-16 flex flex-col justify-between bg-white">
                <div className="space-y-12">
                   <div className="flex justify-between items-start">
                      <div className="space-y-2">
                        <div className="flex items-center gap-3 text-green-600">
                           <ShieldCheck size={24} />
                           <p className="text-[11px] font-black uppercase tracking-[0.4em]">Auth Protocol v5.0</p>
                        </div>
                        <h3 className="text-5xl font-black text-slate-800 outfit tracking-tighter">Bio-Scanner</h3>
                      </div>
                      <button onClick={() => { setIsScannerOpen(false); setProofImage(null); setAiFeedback(null); }} className="p-5 bg-slate-50 text-slate-400 hover:text-slate-800 rounded-3xl transition-all">
                         <X size={32} />
                      </button>
                   </div>
                   ...
                   <div className="space-y-8">
                      <div className="p-8 bg-slate-50 rounded-[3rem] border border-slate-100 shadow-inner">
                         <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">ACTIVE PHASE OBJECTIVE</p>
                         <p className="text-3xl font-black text-slate-800 outfit tracking-tight leading-tight">{currentStep?.title}</p>
                         <p className="text-slate-500 text-lg mt-4 font-medium leading-relaxed italic">"{currentStep?.description}"</p>
                      </div>

                      {aiFeedback && (
                        <div className={`p-10 rounded-[3.5rem] animate-in slide-in-from-bottom-4 duration-700 flex gap-8 shadow-2xl ${
                          aiFeedback.verified ? 'bg-green-600 text-white ring-8 ring-green-100' : 'bg-rose-100 text-rose-800'
                        }`}>
                           <div className="flex-shrink-0 pt-2">
                              {aiFeedback.verified ? <BadgeCheck size={48} /> : <AlertCircle size={48} />}
                           </div>
                           <div className="space-y-2">
                              <p className="font-black text-3xl outfit tracking-tighter leading-none">{aiFeedback.verified ? 'PHASE CLEARED!' : 'RE-SCAN REQUIRED'}</p>
                              <p className={`text-lg font-bold leading-relaxed ${aiFeedback.verified ? 'opacity-90' : 'opacity-80'}`}>{aiFeedback.reasoning}</p>
                           </div>
                        </div>
                      )}
                   </div>
                ...
                <div className="pt-12 space-y-6">
                   {!aiFeedback?.verified ? (
                     <button 
                       ...
                       className="w-full py-8 bg-slate-900 text-white rounded-[2.5rem] font-black text-2xl flex items-center justify-center gap-6 shadow-2xl transition-all active:scale-95 disabled:opacity-30 group relative overflow-hidden"
                     >
                        {verifying ? <Loader2 className="animate-spin" size={32} /> : <Zap size={32} fill="currentColor" className="text-amber-400" />}
                        <span>{verifying ? 'NEURAL MAPPING...' : 'START BIO-SCAN'}</span>
                     </button>
                   ) : (
                     <button onClick={() => { setIsScannerOpen(false); setProofImage(null); setAiFeedback(null); }} className="w-full py-8 bg-green-600 text-white rounded-[2.5rem] font-black text-2xl flex items-center justify-center gap-6 shadow-2xl active:scale-95">
                        <Check size={32} strokeWidth={4} /> COMMIT TO TIMELINE
                     </button>
                   )}
                   <p className="text-[10px] text-center font-black text-slate-400 uppercase tracking-widest opacity-60">Visual verification strictly processed via Gemini 3.0 Pro Vision Engine.</p>
                </div>
             </div>
          </div>
        </div>
