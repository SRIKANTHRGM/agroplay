  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-500 pb-20">
      <div className="text-center space-y-4">
        ...
        <h2 className="text-5xl font-black text-white outfit tracking-tighter leading-none">Value Addition Hub</h2>
        <p className="text-slate-300 max-w-xl mx-auto text-xl font-medium">Transform surplus harvest into elite preserved commodities and earn network XP.</p>
      </div>
        ...
        <div className={`lg:col-span-4 space-y-6 ${isProducing || finished ? 'opacity-40 grayscale pointer-events-none' : ''}`}>
          <div className="flex items-center justify-between px-2">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-2xl font-black text-white outfit tracking-tight">Available Surplus</h3>
            </div>
          </div>
          <div className="space-y-4 max-h-[700px] overflow-y-auto pr-2 custom-scrollbar">
            {MOCK_SURPLUS.map(crop => (
              <button
                ...
                className={`w-full p-6 rounded-[2.5rem] border-4 transition-all text-left flex items-center gap-5 group shadow-sm ${selectedCrop?.id === crop.id
                  ? 'bg-green-600 text-white border-green-500 shadow-xl scale-[1.02]'
                  : 'bg-white border-white hover:border-green-100 hover:bg-green-50/30 text-slate-900'
                  }`}
              >
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest w-fit ${selectedCrop?.id === crop.id ? 'bg-white/20' : 'bg-slate-100 text-slate-700'}`}>
        ...
        <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className={`space-y-6 ${isProducing || finished || !selectedCrop ? 'opacity-40 grayscale pointer-events-none' : ''}`}>
            <div className="flex items-center gap-3 px-2">
              ...
              <h3 className="text-2xl font-black text-white outfit tracking-tight">Preservation Path</h3>
            </div>
            <div className="space-y-4 min-h-[300px]">
              {!selectedCrop ? (
                <div className="bg-white rounded-[3rem] p-12 border-4 border-dashed border-slate-100 flex flex-col items-center justify-center text-center space-y-6 h-full shadow-inner">
                  <div className="w-20 h-20 bg-slate-50 rounded-3xl flex items-center justify-center text-slate-300"><Box size={48} /></div>
                  <p className="text-lg font-bold text-slate-500 outfit leading-relaxed">Select a surplus source<br />to calculate conversions</p>
                </div>
              ) : (
                CONVERSION_RECIPES[selectedCrop.name] && CONVERSION_RECIPES[selectedCrop.name].length > 0 ? (
                  CONVERSION_RECIPES[selectedCrop.name].map(recipe => (
                    <div
                      key={recipe.id}
                      className={`w-full p-8 rounded-[3rem] border-4 transition-all text-left flex flex-col gap-6 group shadow-sm ${selectedRecipe?.id === recipe.id ? 'bg-white border-amber-400 ring-4 ring-amber-50 shadow-2xl' : 'bg-white border-white hover:bg-amber-50/30'
                        }`}
                  ...
                  <div className="bg-white rounded-[3rem] p-12 border-4 border-dashed border-slate-100 flex flex-col items-center justify-center text-center space-y-6 h-full shadow-inner">
                    <div className="w-20 h-20 bg-rose-50 text-rose-500 rounded-3xl flex items-center justify-center"><AlertCircle size={48} /></div>
                    <p className="text-lg font-bold text-slate-400 outfit leading-relaxed">No recipes found for {selectedCrop.name}.<br />Check back later!</p>
                  </div>
                )
              )}
            </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3 px-2">
              ...
              <h3 className="text-2xl font-black text-white outfit tracking-tight">Active Line</h3>
            </div>
            ...
            <div className="bg-white rounded-[4rem] p-12 shadow-2xl shadow-slate-200/50 border border-slate-100 h-[600px] flex flex-col items-center justify-center text-center relative overflow-hidden group">
              {isProducing ? (
                ...
                <div className="space-y-8 opacity-40 group-hover:opacity-60 transition-opacity flex flex-col items-center">
                  ...
                  <div className="space-y-2">
                    <p className="text-3xl font-black outfit text-slate-600 tracking-tighter">Processing Unit Idle</p>
                    <p className="text-slate-500 font-bold uppercase text-[10px] tracking-[0.3em] px-12 leading-relaxed">Commit a preservation path<br />to activate the line</p>
                  </div>
                </div>
              )}
            </div>
