...
const Profile: React.FC<Props> = ({ user, setUser }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 relative px-4">
      {toast && (
        <div className={`fixed top-10 right-10 z-[200] flex items-center gap-3 px-8 py-4 rounded-3xl shadow-2xl animate-in slide-in-from-right-10 border border-white/20 backdrop-blur-xl ${
          toast.type === 'success' ? 'bg-green-600/90 text-white' : 'bg-rose-600/90 text-white'
        }`}>
