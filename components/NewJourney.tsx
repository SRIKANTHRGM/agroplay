          ...
          <div className="bg-white w-full max-w-2xl rounded-[4rem] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-500 relative border-8 border-white">
            <div className="relative h-64 overflow-hidden">
              ...
              <button 
                ...
                className="absolute top-8 right-8 p-4 bg-white/30 backdrop-blur-md hover:bg-white/50 text-white rounded-[1.5rem] transition-all"
              >
                <X size={24} />
              </button>
            </div>

            <div className="p-12 -mt-12 relative bg-white rounded-t-[3.5rem] space-y-10">
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  <h3 className="text-5xl font-black text-slate-800 outfit tracking-tighter leading-none">{selectedCrop.name}</h3>
                  <p className="text-[11px] font-black text-green-600 uppercase tracking-[0.3em]">{selectedCrop.season} Season • {selectedCrop.workflow?.length} Steps</p>
                </div>
                <div className="w-16 h-16 bg-green-50 rounded-[1.5rem] flex items-center justify-center text-green-600 shadow-inner">
                   <Sprout size={32} />
                </div>
              </div>

              <div className="grid grid-cols-3 gap-6">
                 <div className="bg-slate-50 p-5 rounded-3xl border border-slate-100 flex flex-col items-center gap-2">
                    <Droplets size={24} className="text-blue-500" />
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Water Req.</p>
                    <p className="font-bold text-slate-700">{selectedCrop.waterRequirement}</p>
                 </div>
                 <div className="bg-slate-50 p-5 rounded-3xl border border-slate-100 flex flex-col items-center gap-2">
                    <Wind size={24} className="text-green-500" />
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Soil Type</p>
                    <p className="font-bold text-slate-700">{selectedCrop.soilSuitability[0]}</p>
                 </div>
                 <div className="bg-slate-50 p-5 rounded-3xl border border-slate-100 flex flex-col items-center gap-2">
                    <Zap size={24} className="text-amber-500" />
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Yield Bonus</p>
                    <p className="font-bold text-slate-700">+12% XP</p>
                 </div>
              </div>

              <div className="bg-green-50 p-8 rounded-[2.5rem] border-2 border-green-100 shadow-inner relative overflow-hidden group">
                ...
                {loadingSummary ? (
                  ...
                  <div className="space-y-4 relative z-10">
                    <p className="text-green-800 text-lg leading-relaxed font-bold italic text-center">"{summary}"</p>
                  </div>
                )}
              </div>
              ...
              <div className="flex gap-6">
                <button 
                  onClick={() => setIsConfirmOpen(false)}
                  className="flex-1 py-6 bg-slate-100 rounded-[2rem] font-black text-xs uppercase tracking-widest text-slate-400 hover:bg-slate-200 hover:text-slate-600 transition-all active:scale-95"
                >
                  Cancel
                </button>
                <button 
                  onClick={handleConfirmStart}
                  className="flex-[2] py-6 bg-green-600 text-white rounded-[2rem] font-black text-xs uppercase tracking-[0.3em] hover:bg-green-700 shadow-2xl shadow-green-100 transition-all flex items-center justify-center gap-4 active:scale-95 group/btn"
                >
