  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 page-transition pb-24">
      <div className="text-center space-y-4">
        ...
        <h2 className="text-5xl font-black text-slate-800 outfit tracking-tighter">Strategic Crop Planner</h2>
        <p className="text-slate-500 max-w-2xl mx-auto text-xl font-medium">Architect a high-yield season using real-time weather patterns and sub-strata diagnostics.</p>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 relative z-10">
            <div className="lg:col-span-7 space-y-10">
               <div className="space-y-8">
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-4">
                      <div className="relative group flex-1">
                        ...
                        <input type="text" value={location} onChange={(e) => setLocation(e.target.value)} className="w-full pl-16 pr-8 py-7 bg-slate-50 border-none rounded-[2rem] focus:ring-4 focus:ring-green-500/10 transition-all font-black outfit text-2xl shadow-inner placeholder:text-slate-300" placeholder="e.g. Ludhiana, PB" />
                      </div>
                      <button 
                  ...
                  {weather && (
                    <div className="bg-gradient-to-br from-slate-900 to-slate-800 p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden animate-in slide-in-from-top-4 duration-500">
                       ...
                       <div className="relative z-10 space-y-6">
                         <div className="flex items-center justify-between border-b border-white/10 pb-6">
                            <div className="flex items-center gap-6">
                               ...
                               <div>
                                  <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-1">Live Climate Sync</p>
                                  <h4 className="text-4xl font-black outfit tracking-tighter">{weather.temp}</h4>
                               </div>
                            </div>
                            <div className="text-right">
                               <p className="text-lg font-bold text-white/80">{weather.condition}</p>
                               <p className="text-[10px] font-black text-white/40 uppercase tracking-widest mt-1">Humidity: {weather.humidity}</p>
                            </div>
                         </div>
                         <div className="p-6 bg-white/5 rounded-2xl border border-white/10 flex items-start gap-4">
                            <Info size={20} className="text-amber-400 flex-shrink-0 mt-1" />
                            <p className="text-sm font-medium text-white/70 leading-relaxed italic">"{weather.summary}"</p>
                         </div>
                       </div>
                    </div>
                  )}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="relative group">
                      ...
                      <select value={soil} onChange={(e) => setSoil(e.target.value)} className="w-full appearance-none pl-16 pr-12 py-7 bg-slate-50 border-none rounded-[2rem] focus:ring-4 focus:ring-green-500/10 transition-all font-black outfit text-2xl shadow-inner cursor-pointer">
            ...
            <div className="p-12 md:p-24 prose prose-xl prose-slate max-w-none">
              {renderedLines.map((line, i) => {
                ...
                if (t.startsWith('###')) return (
                  <h3 key={i} className="text-4xl font-black text-slate-800 outfit mt-16 mb-10 border-l-8 border-green-500 pl-8 leading-none">
                    {t.replace('###', '').trim()}
                  </h3>
                );
                if (t.startsWith('-')) return (
                  <div key={i} className="flex gap-6 mb-8 items-start animate-in slide-in-from-left-4" style={{ animationDelay: `${i*50}ms` }}>
                    <div className="w-3 h-3 bg-green-400 rounded-full mt-3 flex-shrink-0 shadow-[0_0_10px_#22c55e]" />
                    <div className="text-slate-600 m-0 leading-relaxed font-medium">
                       {t.includes('**') ? (() => {
                         const match = t.match(/\*\*(.*?)\*\*(.*)/);
                         if (match) {
                           return (
                ...
                return <p key={i} className="text-slate-600 text-2xl font-medium leading-relaxed mb-10">{t}</p>;
              })}
            </div>
            
            <div className="bg-slate-900 p-12 flex flex-col md:flex-row items-center justify-between gap-10">
              <div className="flex items-center gap-6">
                <div className="w-20 h-20 bg-amber-400 rounded-[2rem] flex items-center justify-center text-black shadow-2xl shadow-amber-400/20">
                   <Star size={36} fill="currentColor" />
                </div>
                <div className="space-y-1">
                   <p className="text-white font-black outfit text-2xl tracking-tight">Precision Unlock Bonus</p>
                   <p className="text-slate-400 font-medium">Implementing this cycle earns <span className="text-green-400 font-bold">1,200 XP Points</span> & Bio-Scientist Badge.</p>
                </div>
              </div>
              <button className="w-full md:w-auto px-16 py-6 bg-white text-slate-900 rounded-[1.8rem] font-black text-lg hover:bg-slate-50 transition-all shadow-2xl hover:scale-105 active:scale-95 flex items-center justify-center gap-3">
